...
static array_state *get_array_state(PyObject *module) {
...
static int array_resize(arrayobject *self, Py_ssize_t newsize) {
  char *items;
  size_t _new_size;
...
static PyObject *b_getitem(arrayobject *ap, Py_ssize_t i) {
  long x = ((signed char *)ap->ob_item)[i];
  return PyLong_FromLong(x);
}

static int b_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
  short x;
...
static PyObject *BB_getitem(arrayobject *ap, Py_ssize_t i) {
  long x = ((unsigned char *)ap->ob_item)[i];
  return PyLong_FromLong(x);
}

static int BB_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
  unsigned char x;
...
static PyObject *u_getitem(arrayobject *ap, Py_ssize_t i) {
...
static PyObject *w_getitem(arrayobject *ap, Py_ssize_t i) {
  return PyUnicode_FromOrdinal(((Py_UCS4 *)ap->ob_item)[i]);
}

static int w_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
  PyObject *u;
  if (!PyArg_Parse(v, "U;array item must be unicode character", &u)) {
    return -1;
  }
...
static PyObject *h_getitem(arrayobject *ap, Py_ssize_t i) {
  return PyLong_FromLong((long)((short *)ap->ob_item)[i]);
}

static int h_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
  short x;
...
static PyObject *HH_getitem(arrayobject *ap, Py_ssize_t i) {
  return PyLong_FromLong((long)((unsigned short *)ap->ob_item)[i]);
}

static int HH_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
  int x;
...
static PyObject *i_getitem(arrayobject *ap, Py_ssize_t i) {
  return PyLong_FromLong((long)((int *)ap->ob_item)[i]);
}

static int i_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
  int x;
...
static PyObject *II_getitem(arrayobject *ap, Py_ssize_t i) {
  return PyLong_FromUnsignedLong(
      (unsigned long)((unsigned int *)ap->ob_item)[i]);
}

static int II_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
  unsigned long x;
...
static PyObject *LL_getitem(arrayobject *ap, Py_ssize_t i) {
  return PyLong_FromUnsignedLong(((unsigned long *)ap->ob_item)[i]);
}

static int LL_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
  unsigned long x;
  int do_decref = 0; /* if nb_int was called */
...
static PyObject *q_getitem(arrayobject *ap, Py_ssize_t i) {
  return PyLong_FromLongLong(((long long *)ap->ob_item)[i]);
}

static int q_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
  long long x;
  if (!PyArg_Parse(v, "L;array item must be integer", &x))
    return -1;
  if (i >= 0)
...
static PyObject *QQ_getitem(arrayobject *ap, Py_ssize_t i) {
  return PyLong_FromUnsignedLongLong(((unsigned long long *)ap->ob_item)[i]);
}

static int QQ_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
...
static PyObject *f_getitem(arrayobject *ap, Py_ssize_t i) {
  return PyFloat_FromDouble((double)((float *)ap->ob_item)[i]);
}

static int f_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
  float x;
...
static PyObject *d_getitem(arrayobject *ap, Py_ssize_t i) {
  return PyFloat_FromDouble(((double *)ap->ob_item)[i]);
}

static int d_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
  double x;
...
#define DEFINE_COMPAREITEMS(code, type)                                        \
  static int code##_compareitems(const void *lhs, const void *rhs,             \
                                 Py_ssize_t length) {                          \
...
static PyObject *newarrayobject(PyTypeObject *type, Py_ssize_t size,
...
static PyObject *getarrayitem(PyObject *op, Py_ssize_t i) {
#ifndef NDEBUG
...
static int ins1(arrayobject *self, Py_ssize_t where, PyObject *v) {
  char *items;
...
static int array_tp_traverse(arrayobject *op, visitproc visit, void *arg) {
  Py_VISIT(Py_TYPE(op));
  return 0;
}
...
static PyObject *array_richcompare(PyObject *v, PyObject *w, int op) {
...
static Py_ssize_t array_length(arrayobject *a) { return Py_SIZE(a); }
...
static PyObject *array_slice(arrayobject *a, Py_ssize_t ilow,
                             Py_ssize_t ihigh) {
  ...
  if (ihigh < ilow)
    ...
    ihigh = Py_SIZE(a);
  np = (arrayobject *)newarrayobject(state->ArrayType, ihigh - ilow,
                                     a->ob_descr);
  if (np == NULL)
    return NULL;
  if (ihigh > ilow) {
...
static PyObject *array_array___copy___impl(arrayobject *self)
...
static PyObject *array_array___deepcopy__(arrayobject *self, PyObject *unused)
...
#define b ((arrayobject *)bb)
  ...
  if (Py_SIZE(b) > 0) {
    memcpy(np->ob_item + Py_SIZE(a) * a->ob_descr->itemsize, b->ob_item,
...
static int array_del_slice(arrayobject *a, Py_ssize_t ilow, Py_ssize_t ihigh) {
  char *item;
...
static int array_ass_item(arrayobject *a, Py_ssize_t i, PyObject *v) {
  if (i < 0 || i >= Py_SIZE(a)) {
...
static int setarrayitem(PyObject *a, Py_ssize_t i, PyObject *v) {
#ifndef NDEBUG
  array_state *state = find_array_state_by_type(Py_TYPE(a));
...
static int array_iter_extend(arrayobject *self, PyObject *bb) {
  PyObject *it, *v;
...
static int array_do_extend(array_state *state, arrayobject *self,
                           PyObject *bb) {
...
#define b ((arrayobject *)bb)
  ...
  if ((Py_SIZE(self) > PY_SSIZE_T_MAX - Py_SIZE(b)) ||
      ((Py_SIZE(self) + Py_SIZE(b)) >
       PY_SSIZE_T_MAX / self->ob_descr->itemsize)) {
    PyErr_NoMemory();
    return -1;
  }
  ...
  if (array_resize(self, size) == -1)
    return -1;
  if (bbsize > 0) {
    memcpy(self->ob_item + oldsize * self->ob_descr->itemsize, b->ob_item,
...
static PyObject *array_inplace_concat(arrayobject *self, PyObject *bb) {
...
static PyObject *array_inplace_repeat(arrayobject *self, Py_ssize_t n) {
...
static PyObject *array_array_count(arrayobject *self, PyObject *v)
...
static PyObject *array_array_index_impl(arrayobject *self, PyObject *v,
...
static int array_contains(arrayobject *self, PyObject *v) {
  Py_ssize_t i;
  int cmp;
...
static PyObject *array_array_remove(arrayobject *self, PyObject *v)
...
static PyObject *array_array_pop_impl(arrayobject *self, Py_ssize_t i)
...
static PyObject *array_array_extend_impl(arrayobject *self, PyTypeObject *cls,
                                         PyObject *bb)
...
static PyObject *array_array_insert_impl(arrayobject *self, Py_ssize_t i,
                                         PyObject *v)
...
static PyObject *array_array_buffer_info_impl(arrayobject *self)
...
static PyObject *array_array_append(arrayobject *self, PyObject *v)
...
static PyObject *array_array_byteswap_impl(arrayobject *self)
...
static PyObject *array_array_reverse_impl(arrayobject *self)
...
static PyObject *array_array_fromfile_impl(arrayobject *self, PyTypeObject *cls,
                                           PyObject *f, Py_ssize_t n)
...
static PyObject *array_array_tofile_impl(arrayobject *self, PyTypeObject *cls,
                                         PyObject *f)
...
static PyObject *array_array_fromlist(arrayobject *self, PyObject *list)
...
static PyObject *array_array_tolist_impl(arrayobject *self)
...
static PyObject *frombytes(arrayobject *self, Py_buffer *buffer) {
...
if it had been read from a file using the fromfile() method. [clinic start
generated code]*/

static PyObject *array_array_frombytes_impl(arrayobject *self,
                                            Py_buffer *buffer)
...
representation. [clinic start generated code]*/

static PyObject *array_array_tobytes_impl(arrayobject *self)
...
static PyObject *array_array_fromunicode_impl(arrayobject *self, PyObject *ustr)
/*[clinic end generated code: output=24359f5e001a7f2b input=025db1fdade7a4ce]*/
{
  ...
  if (typecode != 'u' && typecode != 'w') {
    PyErr_SetString(PyExc_ValueError, "fromunicode() may only be called on "
  ...
  if (typecode == 'u') {
    ...
    if (ustr_length > 1) {
      ...
      // must not fail
      PyUnicode_AsWideChar(ustr, ((wchar_t *)self->ob_item) + old_size,
                           ustr_length);
    }
  } else { // typecode == 'w'
...
static PyObject *array_array_tounicode_impl(arrayobject *self)
...
static PyObject *array_array___sizeof___impl(arrayobject *self)
 ...
 * be found.
 */
static enum machine_format_code typecode_to_mformat_code(char typecode) {
 ...
 */
static PyObject *make_array(PyTypeObject *arraytype, char typecode,
                            PyObject *items) {
  PyObject *new_args;
  PyObject *array_obj;
...
static PyObject *array__array_reconstructor_impl(
    PyObject *module, PyTypeObject *arraytype, int typecode,
    enum machine_format_code mformat_code, PyObject *items)
/*[clinic end generated code: output=e05263141ba28365 input=2464dc8f4c7736b5]*/
{
  ...
  if (!PyType_IsSubtype(arraytype, state->ArrayType)) {
    PyErr_Format(PyExc_TypeError, "%.200s is not a subtype of %.200s",
  ...
  if (!PyBytes_Check(items)) {
    PyErr_Format(PyExc_TypeError, "fourth argument should be bytes, not %.200s",
  ...
  case SIGNED_INT64_BE: {
     ...
     */
    for (descr = descriptors; descr->typecode != '\0'; descr++) {
      if (descr->is_integer_type && (size_t)descr->itemsize == mf_descr.size &&
    ...
    for (i = 0; i < itemcount; i++) {
      ...
      pylong =
          _PyLong_FromByteArray(&memstr[i * mf_descr.size], mf_descr.size,
                                !mf_descr.is_big_endian, mf_descr.is_signed);
      if (pylong == NULL) {
        Py_DECREF(converted_items);
        return NULL;
      }
...
static PyObject *array_array___reduce_ex___impl(arrayobject *self,
                                                PyTypeObject *cls,
                                                PyObject *value)
/*[clinic end generated code: output=4958ee5d79452ad5 input=19968cf0f91d3eea]*/
{
  ...
  if (PyObject_GetOptionalAttr((PyObject *)self, state->str___dict__, &dict) <
      0) {
    return NULL;
  }
  if (dict == NULL) {
  ...
  result = Py_BuildValue("O(OCiN)O", state->array_reconstructor, Py_TYPE(self),
                         typecode, mformat_code, array_str, dict);
  Py_DECREF(dict);
  return result;
}

static PyObject *array_get_typecode(arrayobject *a, void *closure) {
  char typecode = a->ob_descr->typecode;
  return PyUnicode_FromOrdinal(typecode);
}

static PyObject *array_get_itemsize(arrayobject *a, void *closure) {
...
static PyMethodDef array_methods[] = {
    ...
    ARRAY_ARRAY_COUNT_METHODDEF ARRAY_ARRAY___DEEPCOPY___METHODDEF,
    ARRAY_ARRAY_EXTEND_METHODDEF ARRAY_ARRAY_FROMFILE_METHODDEF,
    ARRAY_ARRAY_FROMLIST_METHODDEF ARRAY_ARRAY_FROMBYTES_METHODDEF,
    ARRAY_ARRAY_FROMUNICODE_METHODDEF ARRAY_ARRAY_INDEX_METHODDEF,
    ARRAY_ARRAY_INSERT_METHODDEF ARRAY_ARRAY_POP_METHODDEF,
    ...
    ARRAY_ARRAY___SIZEOF___METHODDEF{"__class_"
                                     "getitem__",
                                     Py_GenericAlias, METH_O | METH_CLASS,
                                     PyDoc_STR("See PEP "
                                               "585")},
    {NULL, NULL} /* sentinel */
};
...
static PyObject *array_repr(arrayobject *a) {
  ...
  if (len == 0) {
    return PyUnicode_FromFormat("%s('%c')", _PyType_Name(Py_TYPE(a)),
                                (int)typecode);
  }
  if (typecode == 'u' || typecode == 'w') {
  ...
  s = PyUnicode_FromFormat("%s('%c', %R)", _PyType_Name(Py_TYPE(a)),
                           (int)typecode, v);
  Py_DECREF(v);
  return s;
}

static PyObject *array_subscr(arrayobject *self, PyObject *item) {
  array_state *state = find_array_state_by_type(Py_TYPE(self));
  ...
  if (PyIndex_Check(item)) {
    ...
    if (slicelength <= 0) {
      ...
      if (result == NULL)
        return NULL;
      memcpy(((arrayobject *)result)->ob_item, self->ob_item + start * itemsize,
      ...
      for (cur = start, i = 0; i < slicelength; cur += step, i++) {
        memcpy(ar->ob_item + i * itemsize, self->ob_item + cur * itemsize,
               itemsize);
      }
...
static int array_ass_subscr(arrayobject *self, PyObject *item,
                            PyObject *value) {
  ...
  if (step == 1) {
    ...
    if (needed > 0)
      memcpy(self->ob_item + start * itemsize, other->ob_item,
             needed * itemsize);
    return 0;
  } else if (needed == 0) {
    /* Delete slice */
    ...
    for (cur = start, i = 0; i < slicelength; cur += step, i++) {
      ...
      memmove(self->ob_item + (cur - i) * itemsize,
              self->ob_item + (cur + 1) * itemsize, lim * itemsize);
    }
    cur = start + (size_t)slicelength * step;
    if (cur < (size_t)Py_SIZE(self)) {
      memmove(self->ob_item + (cur - slicelength) * itemsize,
              self->ob_item + cur * itemsize, (Py_SIZE(self) - cur) * itemsize);
    }
    ...
    for (cur = start, i = 0; i < slicelength; cur += step, i++) {
      memcpy(self->ob_item + cur * itemsize, other->ob_item + i * itemsize,
             itemsize);
    }
    return 0;
  }
...
static int array_buffer_getbuf(arrayobject *self, Py_buffer *view, int flags) {
  if (view == NULL) {
    PyErr_SetString(PyExc_BufferError,
...
static void array_buffer_relbuf(arrayobject *self, Py_buffer *view) {
  self->ob_exports--;
}
...
static PyObject *array_new(PyTypeObject *type, PyObject *args, PyObject *kwds) {
  ...
  if (!(initial == NULL || PyList_Check(initial) ||
        ...
        PyTuple_Check(initial) || (is_unicode && PyUnicode_Check(initial)) ||
...
static struct PyMemberDef array_members[] = {
    {"__weaklistoffset__", Py_T_PYSSIZET, offsetof(arrayobject, weakreflist),
     Py_READONLY},
    {NULL},
};
...
static PyObject *arrayiter_next(arrayiterobject *it) {
  arrayobject *ao;
...
static int arrayiter_traverse(arrayiterobject *it, visitproc visit, void *arg) {
  Py_VISIT(Py_TYPE(it));
  Py_VISIT(it->ao);
  return 0;
}
...
static PyObject *array_arrayiterator___reduce___impl(arrayiterobject *self,
                                                     PyTypeObject *cls)
...
static PyObject *array_arrayiterator___setstate__(arrayiterobject *self,
                                                  PyObject *state)
...
static PyMethodDef arrayiter_methods[] = {
    ARRAY_ARRAYITERATOR___REDUCE___METHODDEF
        ARRAY_ARRAYITERATOR___SETSTATE___METHODDEF{NULL, NULL} /* sentinel */
};
...
static int array_traverse(PyObject *module, visitproc visit, void *arg) {
  array_state *state = get_array_state(module);
...
static int array_clear(PyObject *module) {
  array_state *state = get_array_state(module);
...
static void array_free(void *module) { array_clear((PyObject *)module); }
...
static int array_modexec(PyObject *m) {
